...
Go to line 147 for the non-functional code
...
def printcount(count_table):
    print("\nThis is the table with bigram counts: ")
    print statement_divided
    for list1 in count_table:
        print list1
...
def printbigram(bigram_table):
    print("\nThis is the table with bigram probabilities: ")
    for list1 in bigram_table:
        print list1
...
count_table = createcount(corpus_divided, statement_divided,
                               count_table)  # calculate the probability for each combo:
printcount(count_table)  # prints probability table
...
printbigram(bigram_table)  # to print bigram table
...
printcount(change_count_table)          #prints probability table
...
printbigram(change_bigram_table)
...
changeTuringValue(dict_freq,new_freq)
new_count_table = newCount(new_freq,count_table)
printcount(new_count_table)         #prints the bigram count
new_bigram_table = createbigram(word_count,statement_divided,new_count_table)
printbigram(new_bigram_table)       #prints the probability count
